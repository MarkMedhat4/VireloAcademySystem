...
import { Lock, LogOut, Pencil, Save } from "lucide-react";
import { Alert } from "@/components/ui/alert";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { SelectField, TextField } from "@/components/ui/field";
import { focusFirstInvalid } from "@/components/forms/form-utils";
import { lookupStudent, studentSignOut, updateMyStudent } from "@/app/actions/student";
...
export function StudentPortal({ initialStudent, configured }: { initialStudent: Student | null; configured: boolean }) {
  ...
  async function enter(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await lookupStudent(normalized);
      if (res.ok) {
        setStudent(res.data);
        setStage("profile");
      } else {
        setErrors(res.fieldErrors ?? {});
        if (!res.fieldErrors?.phone) setMessage({ kind: "error", text: res.message });
        focusFirstInvalid(formRef.current);
      }
    } catch {
  ...
  if (stage === "phone") {
    return (
      <Card accent className="p-6 md:p-8">
        ...
        {alert}
        <form ref={formRef} onSubmit={enter} noValidate className="space-y-5">
          <TextField
            id="phone"
            label="رقم الهاتف"
            required
            ...
            error={errors.phone}
            hint="اكتب رقم هاتف الطالب المسجّل لدينا."
          />
